...
Stary_Kopiec::Stary_Kopiec() {
    Stary_Kopiec::tablica = NULL;
    Stary_Kopiec::rozmiar = 0;
}

Stary_Kopiec::~Stary_Kopiec() {

    if (Stary_Kopiec::rozmiar > 0) {
        delete[]tablica;
    }

}
// TODO refactor kopca
void Stary_Kopiec::dodaj(int wartosc) {

    //Wartości w kopcu nie mogą się powtarzać
    //Kontynuuj tylko, jeżeli wartość nie istnieje jeszcze w kopcu
    if (!Stary_Kopiec::sprawdzCzyIstnieje(wartosc)) {

        //Stwórz nową tablicę większą o jeden inne_elementy
...
void Stary_Kopiec::usun(int wartosc) {
    ...
    for (int i = 0; i < rozmiar; i++) {
        ...
        if (tablica[i] == wartosc) {
            ...
            //Stwórz nową tablicę, pomniejszoną o jeden inne_elementy
            int *nowaTablica = new int[rozmiar - 1];

            //Przepisz elementy z tablicy do pozycji na której znajduje się usuwany inne_elementy
            for (int k = 0; k < i; k++) {
...
bool Stary_Kopiec::sprawdzCzyIstnieje(int wartosc) {
...
bool Stary_Kopiec::czy_sie_powtarza(int wartosc) {
...
void Stary_Kopiec::wydrukujKopiec() {
...
void Stary_Kopiec::poprawStrukture() {
